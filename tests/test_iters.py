...
def test_iters(shape, pshape1, pshape2, dtype):
    ...
    a = cat.from_buffer(bytes(nparray), nparray.shape, itemsize=nparray.itemsize, pshape=pshape1)
...
def test_iters_numpy(shape, pshape1, pshape2, dtype):
    size = int(np.prod(shape))
    nparray = np.ones(size, dtype=dtype).reshape(shape)
    a = cat.from_numpy(nparray, pshape=pshape1)  # creates a NPArray

    b = cat.empty(shape, dtype, pshape=pshape2)
    blockshape = pshape2 if pshape2 is not None else b.pshape
    for (block_r, info_r), (block_w, info_w) in lzip(a.iter_read(blockshape), b.iter_write()):
        block_w[:] = block_r
